...
class Fm_Bogo_Orders {
	...
	public static function save_free_item_meta( $item, $cart_item_key, $cart_item, $order ) {
		if ( ! empty( $cart_item['fm_bogo_free'] ) ) {
			$item->add_meta_data( '_fm_bogo_free_promo_id', (int) $cart_item['fm_bogo_free'], true );
			$item->add_meta_data( '_fm_bogo_promo_label', sanitize_text_field( $cart_item['fm_bogo_promo_label'] ?? '' ), true );
		}
	}
	...
	public static function increment_quota( $order_id ) {
		...
		foreach ( $counts as $promo_id => $qty ) {
			$used = (int) get_post_meta( $promo_id, '_fm_bogo_free_items_used', true );
			update_post_meta( $promo_id, '_fm_bogo_free_items_used', $used + $qty );
			delete_transient( 'fm_bogo_active_promos' );
		}

		$order->update_meta_data( '_fm_bogo_quota_increments', $counts );
		$order->save();
	}
	...
	public static function restore_quota( $order_id ) {
		...
		if ( $order->get_meta( '_fm_bogo_quota_restored' ) ) {
			return;
		}

		$counts = $order->get_meta( '_fm_bogo_quota_increments' );
		...
		foreach ( $counts as $promo_id => $qty ) {
			$used = (int) get_post_meta( $promo_id, '_fm_bogo_free_items_used', true );
			update_post_meta( $promo_id, '_fm_bogo_free_items_used', max( 0, $used - $qty ) );
			delete_transient( 'fm_bogo_active_promos' );
		}

		$order->update_meta_data( '_fm_bogo_quota_restored', '1' );
		$order->save();
	}
	...
	private static function get_free_item_counts( $order ) {
		...
		foreach ( $order->get_items() as $item ) {
			$promo_id = (int) $item->get_meta( '_fm_bogo_free_promo_id' );
			if ( ! $promo_id ) {
				continue;
			}
